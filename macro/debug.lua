...
local function debugLog(message, category)
    ...
    local categoryTag = category and "[" .. category .. "] " or ""

    print(string.format("[PATHFINDING DEBUG] %s %s%s", timestamp, categoryTag, message))

    -- Also show in status for important events
    if category == "ERROR" or category == "STUCK" or category == "TIMEOUT" then
        updateStatus("DEBUG: " .. message:sub(1, 15), Color3.fromRGB(255, 100, 100))
    end
end
...
-- MODIFIED: Fungsi moveToPosition dengan DEBUG SYSTEM
local function moveToPosition(targetPosition, callback)
    if not hrp or not hum then
        debugLog("HRP atau Humanoid tidak ditemukan", "ERROR")
        if callback then callback(false) end
        return false
    end

    if isPathfinding then
        debugLog("Pathfinding sudah berjalan", "WARNING")
        if callback then callback(false) end
        return false
    end
    ...
    if hum.WalkSpeed < 20 then
        ...
        debugLog("WalkSpeed dinaikkan dari " .. originalWalkSpeed .. " ke 20", "INFO")
    end

    local charHeight = getCharacterHeight(character)
    debugLog("Character height: " .. charHeight, "INFO")
    ...
    local targetDistance = (startPos - targetPosition).Magnitude
    debugLog(string.format("Pathfinding dari %s ke %s (Jarak: %.2f)",
        tostring(startPos), tostring(targetPosition), targetDistance), "START")

    local path = PathfindingService:CreatePath(pathParams)

    -- Compute path dengan error handling
    local computeSuccess, computeError = pcall(function()
        path:ComputeAsync(startPos, targetPosition)
    end)

    if not computeSuccess then
        debugLog("Error saat compute path: " .. tostring(computeError), "ERROR")
        isPathfinding = false
        macroLocked = false
    ...
    debugLog("Path status: " .. tostring(path.Status), "INFO")

    -- Compute path
    if path.Status ~= Enum.PathStatus.Success then
        debugLog("Path computation gagal dengan status: " .. tostring(path.Status), "ERROR")
        isPathfinding = false
        macroLocked = false
    ...
    local function isStuck(lastPos, newPos, threshold)
        local distanceMoved = (lastPos - newPos).Magnitude
        local isStuck = distanceMoved < threshold
        if isStuck then
            debugLog(string.format("Karakter stuck! Hanya bergerak %.3f stud", distanceMoved), "STUCK")
        end
        return isStuck
    end
    ...
    if path.Status == Enum.PathStatus.Success then
        ...
        debugLog("Path berhasil dengan " .. totalWaypoints .. " waypoints", "SUCCESS")

        -- loop cek posisi & anti-stuck
        local stuckCheckConnection
        stuckCheckConnection = RunService.Heartbeat:Connect(function()
            ...
            if isStuck(lastPos, currentPos, stuckThreshold) then
                ...
                debugLog(string.format("Stuck timer: %.1f/%ds", stuckTime, stuckTimeout), "STUCK")
                if stuckTime >= stuckTimeout then
                    debugLog("Triggering jump untuk mengatasi stuck", "ACTION")
                    hum:ChangeState(Enum.HumanoidStateType.Jumping)
                    stuckTime = 0
                end
            else
                if stuckTime > 0 then
                    debugLog("Karakter berhasil keluar dari stuck state", "RECOVERY")
                    stuckTime = 0
                end
            end
            lastPos = currentPos
        ...
        for i, waypoint in ipairs(waypoints) do
            if not isPathfinding then
                debugLog("Pathfinding dibatalkan", "INFO")
                break
            end

            waypointIndex = i
            local action = waypoint.Action == Enum.PathWaypointAction.Jump and "JUMP" or "WALK"
            debugLog(string.format("Waypoint %d/%d: %s (%s)", i, totalWaypoints, tostring(waypoint.Position), action),
                "WAYPOINT")

            if waypoint.Action == Enum.PathWaypointAction.Jump then
                debugLog("Melompat ke waypoint " .. i, "ACTION")
                hum:ChangeState(Enum.HumanoidStateType.Jumping)
            end

            hum:MoveTo(waypoint.Position)

            -- Wait for movement completion dengan timeout
            local moveCompleted = false
            local moveStartTime = tick()
            local moveTimeout = 10 -- 10 detik timeout per waypoint

            while not moveCompleted and isPathfinding do
                local moveSuccess = hum.MoveToFinished:Wait(1) -- Wait dengan timeout 1 detik
                if moveSuccess then
                    moveCompleted = true
                    debugLog("Waypoint " .. i .. " reached", "SUCCESS")
                elseif tick() - moveStartTime > moveTimeout then
                    debugLog("Timeout mencapai waypoint " .. i, "TIMEOUT")
                    break
                end

                -- Cek jika karakter terlalu lama di satu waypoint
                if tick() - moveStartTime > 5 then
                    debugLog("Lama di waypoint " .. i .. ", coba jump", "RETRY")
                    hum:ChangeState(Enum.PathWaypointAction.Jump)
                end
            end

            if not moveCompleted then
                debugLog("Gagal mencapai waypoint " .. i, "ERROR")
                break
            end
        end

        -- Cleanup stuck checker
        if stuckCheckConnection then
            stuckCheckConnection:Disconnect()
        end

        if totalWaypoints == 0 then
            debugLog("Tidak ada waypoints yang tersedia", "ERROR")
            isPathfinding = false
            macroLocked = false
        ...
        debugLog(string.format("Final distance: %.2f (Tolerance: %.1f)", finalDistance, finalTolerance), "COMPLETION")
        ...
        if adjustedWalkSpeed then
            hum.WalkSpeed = originalWalkSpeed
            debugLog("WalkSpeed dikembalikan ke " .. originalWalkSpeed, "INFO")
        end

        if finalDistance <= finalTolerance then
            debugLog("Pathfinding berhasil mencapai target", "SUCCESS")
            updateStatus("READY", Color3.fromRGB(100, 255, 100))
            if callback then callback(true) end
            return true
        else
            debugLog("Gagal mencapai target, jarak terlalu jauh", "ERROR")
            updateStatus("TARGET FAILED", Color3.fromRGB(255, 100, 100))
            if callback then callback(false) end
            return false
        end
    else
        debugLog("Path status tidak success: " .. tostring(path.Status), "ERROR")
        isPathfinding = false
        macroLocked = false
        -- RESTORE WALKSPEED asli
        if adjustedWalkSpeed then
...
local function teleportToPosition(targetPosition)
    if not hrp then
        debugLog("HRP tidak ditemukan untuk teleport", "ERROR")
        return false
    end

    debugLog("Memulai teleport ke: " .. tostring(targetPosition), "TELEPORT")
    ...
    local success = distance <= 5

    if success then
        debugLog("Teleport berhasil, jarak: " .. string.format("%.2f", distance), "SUCCESS")
    else
        debugLog("Teleport gagal, jarak: " .. string.format("%.2f", distance), "ERROR")
    end
...
local function findNearestSample()
    if not hrp or #samples == 0 then
        debugLog("Tidak bisa mencari sample terdekat - HRP atau samples tidak ada", "ERROR")
        return 1
    end
    ...
    debugLog(string.format("Sample terdekat: index %d, jarak %.2f", nearestIndex, minDistance), "INFO")
    return nearestIndex, minDistance
end

-- MODIFIED: Fungsi untuk move ke posisi sample terdekat dengan DEBUG
local function moveToSamplePosition(targetIndex, callback)
    if not hrp or not samples[targetIndex] or not samples[targetIndex].cf then
        debugLog("Data tidak valid untuk moveToSamplePosition", "ERROR")
        if callback then callback(false) end
        return false
    end
    ...
    debugLog(string.format("Move to sample %d, jarak: %.2f", targetIndex, distance), "INFO")

    -- Jika sudah dekat, tidak perlu melakukan apa-apa
    if distance <= 3 then
        debugLog("Sudah dekat dengan target, skip pathfinding", "INFO")
        if callback then callback(true) end
        return true
    end
    ...
    if distance > 40 then
        ...
        debugLog("Jarak > 40 stud, menggunakan teleport", "DECISION")
    ...
    debugLog("Menggunakan pathfinding untuk jarak menengah", "DECISION")
    updateStatus("PATHFINDING", Color3.fromRGB(255, 200, 50))
    return moveToPosition(targetPosition, function(success)
        if success then
            debugLog("Pathfinding ke sample berhasil", "SUCCESS")
            if callback then callback(true) end
        else
            -- Fallback teleport jika pathfinding gagal
            debugLog("Pathfinding gagal, mencoba teleport fallback", "FALLBACK")
            updateStatus("TELEPORTING", Color3.fromRGB(255, 150, 50))
            local teleportSuccess = teleportToPosition(targetPosition)
            if teleportSuccess then
                debugLog("Teleport fallback berhasil", "SUCCESS")
                updateStatus("TELEPORT OK", Color3.fromRGB(100, 255, 100))
                if callback then callback(true) end
            else
                debugLog("Semua metode gagal", "ERROR")
...
local function moveToNearestSample(callback)
    ...
    if isStartingFromNearest then
        if distance > 40 then
            ...
            debugLog("Starting from nearest CP dengan teleport", "INFO")
        else
            updateStatus("PATHFIND CP", Color3.fromRGB(100, 200, 255))
            debugLog("Starting from nearest CP dengan pathfinding", "INFO")
        end
    else
        if distance > 40 then
            updateStatus("TELEPORT START", Color3.fromRGB(200, 150, 255))
            debugLog("Starting from start dengan teleport", "INFO")
        else
            updateStatus("PATHFIND START", Color3.fromRGB(100, 200, 255))
            debugLog("Starting from start dengan pathfinding", "INFO")
        end
    end

    return moveToSamplePosition(targetIndex, function(success)
        if success then
            debugLog("Berhasil mencapai sample terdekat", "SUCCESS")
            if callback then callback(true, targetIndex) end
        else
            debugLog("Gagal mencapai sample terdekat, mencoba start position", "FALLBACK")
            ...
            if isStartingFromNearest then
                ...
                moveToSamplePosition(1, function(retrySuccess)
                    if retrySuccess then
                        debugLog("Berhasil mencapai start position", "SUCCESS")
                        if callback then callback(true, 1) end
                    else
                        debugLog("Gagal mencapai start position", "ERROR")
                        if callback then callback(false, 1) end
                    end
                end)
            else
                debugLog("Sudah di start position tapi tetap gagal", "ERROR")
                if callback then callback(false, 1) end
            end
        end
    end)
...
-- Playback functions dengan DEBUG
local function startPlayback()
    if #samples < 2 then
        debugLog("Tidak ada data samples yang cukup", "ERROR")
        updateStatus("NO DATA", Color3.fromRGB(255, 150, 50))
        return
    end

    if isPathfinding then
        debugLog("Masih dalam proses pathfinding", "WARNING")
    ...
    playing = true
    macroLocked = true
    debugLog("Memulai playback", "PLAYBACK")

    if needsPathfinding then
        updateStatus("FINDING START", Color3.fromRGB(150, 200, 255))
        debugLog("Mencari start position dengan pathfinding", "INFO")

        moveToNearestSample(function(success, startIndex)
            if success then
                debugLog("Berhasil mencapai start position, index: " .. startIndex, "SUCCESS")
                ...
                if playing then
                    if startFromNearest then
                        ...
                        debugLog("Mulai playback dari checkpoint", "PLAYBACK")
                    else
                        updateStatus("PLAYING", Color3.fromRGB(50, 150, 255))
                        debugLog("Mulai playback dari start", "PLAYBACK")
                    end
                end
            else
                debugLog("Gagal mencapai start position, mencoba final teleport", "FALLBACK")
                playing = false
                macroLocked = false
                ...
                if teleportSuccess then
                    debugLog("Final teleport berhasil", "SUCCESS")
                    updateStatus("TELEPORT OK", Color3.fromRGB(100, 255, 100))
                    playIndex = 1
                    ...
                    if playing then
                        updateStatus("PLAYING", Color3.fromRGB(50, 150, 255))
                        debugLog("Mulai playback setelah final teleport", "PLAYBACK")
                    end
                else
                    debugLog("Final teleport juga gagal", "ERROR")
                    updateStatus("ALL FAILED", Color3.fromRGB(255, 100, 100))
                end
            end
        end)
    else
        debugLog("Langsung mulai playback tanpa pathfinding", "PLAYBACK")
        updateStatus("PLAYING", Color3.fromRGB(50, 150, 255))
    end
end

local function stopPlayback()
    debugLog("Menghentikan playback", "PLAYBACK")
    playing = false
    macroLocked = false
    isPathfinding = false
    if hum then
...
local function resetPlayback()
    debugLog("Reset playback", "PLAYBACK")
    playbackTime = 0
    playIndex = 1
    playing = false
    macroLocked = false
...
-- Tambahkan debug button untuk testing
local debugBtn = createBtn("🐛 DEBUG", UDim2.new(0.05, 0, 0, 315), UDim2.new(0.45, 0, 0, 20), function()
    debugEnabled = not debugEnabled
    if debugEnabled then
        updateStatus("DEBUG ENABLED", Color3.fromRGB(100, 255, 100))
        debugLog("Debug system diaktifkan", "SYSTEM")
    else
        updateStatus("DEBUG DISABLED", Color3.fromRGB(255, 150, 50))
    end
end, Color3.fromRGB(100, 100, 100))

-- Debug info display
local debugInfo = Instance.new("TextLabel", ContentFrame)
debugInfo.Text = "Debug: OFF"
debugInfo.Size = UDim2.new(0.45, 0, 0, 15)
debugInfo.Position = UDim2.new(0.5, 0, 0, 315)
debugInfo.TextColor3 = Color3.fromRGB(150, 150, 150)
debugInfo.BackgroundTransparency = 1
debugInfo.Font = Enum.Font.Gotham
debugInfo.TextSize = 9
debugInfo.TextXAlignment = Enum.TextXAlignment.Right
...
RunService.Heartbeat:Connect(function()
    ...
    -- Update debug info
    if debugEnabled then
        debugInfo.Text = "Debug: ON"
        debugInfo.TextColor3 = Color3.fromRGB(100, 255, 100)
    else
        debugInfo.Text = "Debug: OFF"
        debugInfo.TextColor3 = Color3.fromRGB(150, 150, 150)
    end
...
debugLog("Macro Loader initialized", "SYSTEM")
debugLog("Game ID: " .. getCurrentGameId(), "SYSTEM")
